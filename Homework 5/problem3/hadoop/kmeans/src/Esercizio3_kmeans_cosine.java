...
public class Esercizio3_kmeans_cosine {
	...
	private static boolean initCentroids(int numReducers, Path in, Path out) throws Exception {
		...
		job.setJobName("Esercizio3_kmeans_cosine-initCentroids");
		...
		job.setJarByClass(Esercizio3_kmeans_cosine.class);
	...
	private static boolean updateCentroids(int numReducers, Path in, Path out) throws Exception {
		...
		job.setJobName("Esercizio3_kmeans_cosine-updateCentroids");
	...
	private static boolean writeOutput(int numReducers, Path in, Path out) throws Exception {
		...
		job.setJobName("Esercizio3_kmeans_cosine-writeOutput");
	...
	public static class Mapper_updateCentroids extends Mapper<LongWritable, Text, IntWritable, Text> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			int cluster_index = 1;
			int i = 1;					
			double min_cos_distance = Double.NEGATIVE_INFINITY;
			...
			for (HashMap<String, Double> centroid : centroids) {
				...
				for (String stem : tmp1) {						
					double product = docMap.get(stem) * centroid.get(stem);
					currentDistance += product;											
				}

				tmp1.clear();
				tmp2.clear();

				if (currentDistance > min_cos_distance) {				
					min_cos_distance = currentDistance;
					cluster_index = i;
				}
	...
	public static class Reducer_updateCentroids extends Reducer<IntWritable, Text, Text, Text> {
		...
		protected void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			double square_sum = 0.0;	// normalization
			for (String stem : tempMap.keySet()) {
				double tfidf = tempMap.get(stem) / cluster_dim;
				tempMap.put(stem, tfidf);
			...
			for (String stem : tempMap.keySet()) {
				newCentroid.append(stem + ":" + (tempMap.get(stem) / square_sum) + ",");
			}
